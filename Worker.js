...
async function handleStockAiAnalysis(url, env, corsHeaders) {
  ...
  const lang = (url.searchParams.get('lang') || 'zh').toLowerCase() === 'en' ? 'en' : 'zh';
  const forceRefresh = url.searchParams.get('refresh') === '1' || url.searchParams.get('refresh') === 'true';
  const cacheKey = `stock-ai-analysis:v8:${STOCK_SCORE_MODEL_VERSION}:${NEWS_SOURCE_VERSION}:${lang}:${symbol}`;
  const edgeCached = forceRefresh ? null : await readCachedNews(cacheKey, corsHeaders);
  if (edgeCached) return edgeCached;
  const cached = cache.get(cacheKey);
  if (!forceRefresh && cached && Date.now() - cached.ts < STOCK_AI_CACHE_TTL) {
    return json(cached.data, 200, corsHeaders, { 'X-Cache': 'HIT' });
  }
...
async function fetchPriceCloses(symbol, env) {
  ...
  try {
    ...
    if (response.ok) {
      const payload = await response.json();
      const yahooCloses = normalizePriceCloses(payload?.chart?.result?.[0]?.indicators?.quote?.[0]?.close, 'Yahoo Finance historical closes');
      if (yahooCloses) return yahooCloses;
    }
  } catch (_) {}
  ...
  const finnhubCloses = normalizePriceCloses(finnhub?.c, 'Finnhub historical closes');
  if (finnhub?.s === 'ok' && finnhubCloses) return finnhubCloses;
...
async function buildPriceTrendRow(symbol, env) {
  const candles = await fetchPriceCloses(symbol, env);
  const dailyChanges = [];
  for (let i = 1; i < candles.closes.length; i += 1) {
    const close = candles.closes[i];
    const previous = candles.closes[i - 1];
  ...
  if (dailyChanges.length < 5 || candles.closes.length < 6) throw new Error('Insufficient historical close data');
  const last = dailyChanges[dailyChanges.length - 1];
  const latestClose = candles.closes[candles.closes.length - 1];
  const fiveDayBase = candles.closes[candles.closes.length - 6];
...
async function handleFundFlow(url, env, corsHeaders) {
  ...
  const results = await Promise.all(symbols.map(async symbol => {
    try { return await buildPriceTrendRow(symbol, env); }
    catch (error) { return { symbol, sector: FUND_FLOW_UNIVERSE[symbol], error: error.message }; }
  }));
  results.sort((a, b) => {
  ...
  return json({ model: 'historical-close-price-change-v1', dataType: 'historical-close-price-changes', items: results }, 200, corsHeaders);
}
